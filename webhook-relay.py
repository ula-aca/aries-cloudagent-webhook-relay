...
parser.add_argument(
    ...
    action='store',
    default='0.0.0.0',
    help='the host the relay will run on'
)
parser.add_argument(
    '--port',
    '-p',
    action='store',
    default=8080,
    help='the port the relay will run on'
)
...
async def on_ws_connection(request):
    ...
    if not args.insecure_mode:
        if request.headers.get('Authorization', None) is None:
            logging.warning(
                'denied connection attempt because no key was provided and app is running in secure mode.')
            logging.warning(
                'run this app with the --insecure-mode flag to disable this security check.')
        ...
        if provided_key != args.api_key:
            logging.warning(
                f'denied connection attempt with invalid api key {provided_key}')
            await ws.close(code=aiohttp.WSCloseCode.PROTOCOL_ERROR)
            return
...
if __name__ == '__main__':
    ...
    if args.insecure_mode is False:
        if args.api_key is None:
            ...
            logging.info(
                'both the --api-key and --insecure-mode flags are not provided')
            logging.info(f'generated api key: {args.api_key}')
    else:
        logging.warning(
            'running in insecure mode, don\'t use this in production!')
